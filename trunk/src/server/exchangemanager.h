...
public:
    ...
    /** Removes items from position 'itemNum'. Can return NULL if there is no item.
     * Returned item instance now belongs to caller.
     * @param Count=-1 means take everything.
    ...
    /** Constructs an XML list of the items that are being offered.  The format of
      * the xml is:
    ...
    /** Constructs a simple XML list of the items that are being offered.  The format of
      * the xml is when exact:
...
/** A basic exchange.  This shares the qualities of any exchange and stores the
...
public:
    ...
    /** One of the client has accepted the exchange. If both clients have accepted
    ...
    /** Add a new item to the exchange. Will add to the offering slot of the person that
     ...
     *  @param remain [CHANGES] The number of items left in the slot.
     *
     *  @param A new psItem for the items removed.
     */
