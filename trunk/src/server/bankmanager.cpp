...
void BankManager::ProcessTax()
{
    ...
    for(uint i=0; i < monEvts.GetSize(); i++)
    {
        ...
        if(temp.nextEvent < time)
        {
            ...
            if(temp.guild)
            {
                psGuildInfo* g = CacheManager::GetSingleton().FindGuild(temp.taxid);
                if(!g)
                {
...
void BankManager::SendBankWindow(Client* client, bool guild, bool forceOpen)
{
    ...
    // Not leader (will change later on to have flexible permissions).
    if(guild && pschar->GetGuildLevel()->level != 9)
    {
...
void BankManager::HandleBanking( MsgEntry *me, Client *client )
{
    ...
    // Not leader (will change later on to have flexible permissions).
    if(msg.guild && client->GetCharacterData()->GetGuildLevel()->level != 9)
    {
        psserver->SendSystemError(client->GetClientNum(), "You cannot access your guild bank account without being the leader of your guild!");
        return;
    }
