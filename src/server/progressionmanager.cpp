...
public:
    ...
    float GetCurrentValue(psCharacter * targetChar, bool findBase = false)
    {
        ...
        case STA:
            if (base || findBase)
                return targetChar->GetAttributes()->GetStat(statToAttrib[stat], false);
            else
                return targetChar->GetAttributes()->GetStat(statToAttrib[stat], true);
            break;
        case ATTACK:
...
public:
    ...
    bool Run(gemActor *actor, gemObject *target, bool inverse)
    {
        ...
        else            
        {
            ...
            character->GetSkills()->SetSkillRank(skill, (int) result);
        }
    ...
    switch ( msg.command )
    {
        ...
        case psGUISkillMessage::SKILL_SELECTED:
        {
            ...
            psGUISkillMessage newmsg(client->GetClientNum(),
                            ...
                            buff,
                            NULL,
                            int(chr->GetAttributes()->GetStat(PSITEMSTATS_STAT_STRENGTH)),
                            int(chr->GetAttributes()->GetStat(PSITEMSTATS_STAT_ENDURANCE)),
                            int(chr->GetAttributes()->GetStat(PSITEMSTATS_STAT_AGILITY)),
                            int(chr->GetAttributes()->GetStat(PSITEMSTATS_STAT_INTELLIGENCE)),
                            int(chr->GetAttributes()->GetStat(PSITEMSTATS_STAT_WILL)),
                            int(chr->GetAttributes()->GetStat(PSITEMSTATS_STAT_CHARISMA)),
                            int(chr->GetHP()),
                            int(chr->GetMana()),
                            int(chr->GetStamina(true)),
                            int(chr->GetStamina(false)),
                            int(chr->GetHitPointsMax()),
                            int(chr->GetManaMax()),
                            int(chr->GetStaminaMax(true)),
                            int(chr->GetStaminaMax(false)),
                            true,
                            PSSKILL_NONE,
                            -1,
                            false);
        ...
        case psGUISkillMessage::BUY_SKILL:
        {
            ...
            int current = character->GetSkills()->GetSkillRank((PSSKILL)info->id, false);
...
void ProgressionManager::SendSkillList(Client * client, bool forceOpen, PSSKILL focus, bool isTraining )
{
    ...
    for (int skillID = 0; skillID < (int)PSSKILL_COUNT; skillID++)
    {
        ...
        if  (
                !trainerInfo
                    ||
                trainerInfo->TrainingInSkill((PSSKILL)skillID, character->GetSkills()->GetSkillRank((PSSKILL)skillID, false), faction)
             )
        {
            ...
            */
            ...
            { 
                ...
                {
                    actualStat=character->GetAttributes()->GetStat(PSITEMSTATS_STAT_INTELLIGENCE);
                }
    ...
    psGUISkillMessage newmsg(client->GetClientNum(),
                            ...
                            "",
                            skills,
                            (unsigned int)character->GetAttributes()->GetStat(PSITEMSTATS_STAT_STRENGTH),
                            (unsigned int)character->GetAttributes()->GetStat(PSITEMSTATS_STAT_ENDURANCE),
                            (unsigned int)character->GetAttributes()->GetStat(PSITEMSTATS_STAT_AGILITY),
                            (unsigned int)character->GetAttributes()->GetStat(PSITEMSTATS_STAT_INTELLIGENCE),
                            (unsigned int)character->GetAttributes()->GetStat(PSITEMSTATS_STAT_WILL),
                            (unsigned int)character->GetAttributes()->GetStat(PSITEMSTATS_STAT_CHARISMA),
