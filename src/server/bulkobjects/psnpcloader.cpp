...
bool psNPCLoader::ReadBasicInfo()
{
    ...
    psRaceInfo* raceInfo = CacheManager::GetSingleton().GetRaceInfoByNameGender(race, gender);
    if (!raceInfo)
    {
...
void psNPCLoader::ReadTraits()
{
    ...
    psTrait* faceTrait   = CacheManager::GetSingleton().GetTraitByName(face);
    psTrait* hairTrait  = CacheManager::GetSingleton().GetTraitByName(hair);
    psTrait* beardTrait = CacheManager::GetSingleton().GetTraitByName(beard);
    psTrait* skinTrait  = CacheManager::GetSingleton().GetTraitByName(skin);
...
void psNPCLoader::ReadMerchantInfo()
{
    ...
    else
    {
        ...
        while(iter->HasNext())
        {
            ...
            psItemCategory* itemCategory = CacheManager::GetSingleton().GetItemCategoryByName(name);
...
void psNPCLoader::SetupEquipment()
{
    ...
    while(iter->HasNext())
    {
        ...
        psItemStats * stat = CacheManager::GetSingleton().GetBasicItemStatsByName(name);
        if ( !stat )
        {
        ...
        if (!guild.IsEmpty())
        {
             psGuildInfo* newguild=CacheManager::GetSingleton().FindGuild(guild.GetData());
...
void psNPCLoader::WriteLocation()
{
    ...
    posNode->SetAttributeAsFloat("x", npc->location.loc_x);
    posNode->SetAttributeAsFloat("y", npc->location.loc_y);
    posNode->SetAttributeAsFloat("z", npc->location.loc_z);
...
void psNPCLoader::WriteSkills()
{
    ...
    for (int i=0;i<PSSKILL_COUNT;i++)
    {
        ...
        if (rank)
        {
            ...
            psSkillInfo* skillInfo = CacheManager::GetSingleton().GetSkillByID(i);
